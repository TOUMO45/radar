...
export function VerdictMathBar({ v }: { v: SceneVerdict }) {
  ...
  const tone = held ? "var(--color-status-held)" : "var(--color-status-locked)";
  ...
  return (
    ...
    >
      <div
        className={`hud flex items-center gap-3 px-4 py-3 border-b ${held ? "breathe" : ""}`}
        style={{
          background: "var(--color-bg-raise)",
          borderLeft: `3px solid ${tone}`,
          ["--breathe-color" as string]: tone,
        }}
      >
        <VerdictChip verdict={v.verdict} />
        <span className="text-[var(--color-text-secondary)]">{REASON_COPY[v.reason]}</span>
        <span className="ml-auto mono text-[11px] text-[var(--color-text-faint)]">{i.snapshot_ref}</span>
      </div>

      <div className="flex flex-wrap">
        <Seg k="open blocking ≥ τ" v={String(i.blocking_open)} alert={i.blocking_open > 0} />
        <Seg
          k="gate coverage"
        ...
        <Seg k="kill switch" v={i.kill_switch ? "ENGAGED" : "clear"} alert={i.kill_switch} />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="vmb-seg">
      ...
      <span
        ...
        style={{
          color: alert ? "var(--color-status-error)" : "var(--color-text-primary)",
          textShadow: alert ? "0 0 14px rgba(244,88,107,0.55)" : "none",
        }}
      >
        {v}
      </span>
